...
public class StatusImage {
	...
	public static InputStream drawImage(PlayerChampion rank, Player player) {
		...
		try {
			...
			DirectoryStream<Path> dStream = Files.newDirectoryStream(Paths.get(OusuCore.getAssetsPath() + "/backgrounds/"));
			...
			image.drawImage(championImage.toFile(), 0, 0, new Dimension(900, 239), ImageBuilder.Alignment.Bottom_left);
			image.drawImage(new File(OusuCore.getAssetsPath() + "/BestChampionOverlayer.png"), 0, 0,
			...
			image.drawImage(
					new File(OusuCore.getAssetsPath() + "/elos/"
							+ player.getTier().name() + ".png"),
					799, 103, new Dimension(135, 135), ImageBuilder.Alignment.Center);
